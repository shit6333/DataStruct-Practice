...
class DataProcess{
    public:
        ...
        string SetName( int title_length , string origin_name, string new_head, bool output );            // 輸出新檔案名稱 ( 原檔開頭字元數, 原始檔案名, 新檔案開頭 )
...
class FileTask{
    public:
        ...
        void Combine( vector<vector<string>> & data_vec1, vector<vector<string>> & data_vec2, bool output, bool first_time, string & output_name );
	...
	do{
		...
		switch( command ){
			...
			case 3:{
			    ...
			    string output_name = "";
			    ft.Combine( ft.data_vec_1, ft.data_vec_2 , true, true, output_name );
			...
			case 4:{
                ...
                for( int i=0 ; i < M ; i++ ){
                    ...
                    // Combine
                    vector<vector<string>> data_vec_2;
                    ft.ReadFile(open_file2, data_vec_2, ft.name2 );
                    if( M == 1){
                        ft.Combine( ft.data_vec_1, data_vec_2 , true, true, output_name );
                        break;
                    }

                    if( i == M-1 ){
                        ft.Combine( ft.data_vec_1, data_vec_2 , true, false, output_name );
                    }
                    else{
                        ft.Combine( ft.data_vec_1, data_vec_2 , false, true, output_name );
                    }
                }
...
void FileTask::Combine( vector<vector<string>> & data_vec1, vector<vector<string>> & data_vec2, bool output, bool first_time, string & output_name ){
    // 合併檔案
    ...
    if(first_time){
        output_name = dp.SetOuputTitle( name1 );
        if(!output) output_name = dp.SetName( 4, name2, output_name, false);
    }

    if(output){

        // 設定輸出檔案名
        output_name = dp.SetName( 4, name2, output_name, true);
        ...
        output_name = dp.SetName( 4, name2, output_name, false);
    }
...
void FileTask::Copy( vector<vector<string>> & data_vec ){
    ...
    // 輸出
    string output_name = dp.SetName( 5, name1, "copy",true );
    ofstream  outfile( output_name );
...
void FileTask::Select( vector<vector<string>> & data_vec,int num_students, int num_graduates ){
    ...
    // 輸出
    string output_name = dp.SetName( 4, name1, "copy", true );
    ofstream  outfile( output_name );
    dp.OuputData( data_vec, output_name );
}
...
// 設定輸出檔案名
string DataProcess::SetName( int title_length , string origin_name, string new_head, bool output ){
    ...
    if(output){
        new_name = new_head + "_" + new_name + ".txt";
    }
    else{
        new_name = new_head + "_" + new_name;
    }
